...
public class TransactionsGrid extends Div {
	...
	private final GridListDataView<TransactionDTO> gridDataView = grid.setItems();
	...
	private void applyFilter() {
		AssetDTO selectedAsset = nameSearchField.getValue();
		Set<TransactionType> selectedTypes = typeSearchField.getSelectedItems();

		gridDataView.setFilter(transaction -> {
			boolean nameFilter = selectedAsset == null || transaction.getAssetSymbol().equals(selectedAsset.getSymbol());
	...
	public void setItems(List<TransactionDTO> transactions) {
	...
	private LitRenderer<TransactionDTO> priceColumnRenderer() {
		return LitRenderer.<TransactionDTO>of("<p class='asset-price'>${item.price}</p>")
				.withProperty("price", t -> CurrencyFormatter.withDefaults().format(t.getMarketPrice()));
	}

	private LitRenderer<TransactionDTO> quantityColumnRenderer() {
		...
		return LitRenderer.<TransactionDTO>of("<p class='${item.className}'> ${item.quantity} ${item.symbol}</p>")
				...
				.withProperty("symbol", TransactionDTO::getAssetSymbol);
	}

	private LitRenderer<TransactionDTO> profitLossColumnRenderer() {
		return LitRenderer.<TransactionDTO>of("<div class='transaction-profit-loss ${item.className}'>" +
	...
	private NumberRenderer<TransactionDTO> priceColumnRenderer(ValueProvider<TransactionDTO, Number> priceProvider) {
