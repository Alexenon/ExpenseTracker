...
public class ExportTransactionDialog extends Dialog implements HasNotifications {
	...
	public ExportTransactionDialog(PortfolioDTO portfolio, InstrumentsFacadeService instrumentsFacadeService) {
	...
	private void initialize() {
		...
		optionsSelector.addValueChangeListener(e -> {
			...
			switch (value) {
				case "All" -> updateDownloadedItems(instrumentsFacadeService.getTransactions(portfolio.getId()));
	...
	public void updateDownloadedItems(LocalDate from, LocalDate to) {
		updateDownloadedItems(instrumentsFacadeService.getTransactions(portfolio.getId(), from, to));
	}

	public void updateDownloadedItems(List<TransactionDTO> transactions) {
